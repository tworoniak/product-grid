...
- Color and size selection use the full ARIA `radiogroup`/`radio` pattern, including roving tabindex and arrow-key navigation (Left/Right, Up/Down, Home/End move focus *and* selection together, matching the ARIA Authoring Practices radio pattern) — not just a `role="group"` of independently-tabbable buttons.
